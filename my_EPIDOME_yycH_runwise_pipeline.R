...
pathF = "cutadapted_yycH"
pathR = "cutadapted_yycH"
...
# Filtering  #should include  truncLen=c(278,262), truncLen=c(420,600)
out <- filterAndTrim(fwd=file.path(pathF, fastqFs), filt=file.path(filtpathF, fastqFs),
                     rev=file.path(pathR, fastqRs), filt.rev=file.path(filtpathR, fastqRs),
                      maxEE=10, truncQ=2, maxN=0, rm.phix=TRUE, minLen=200,
...
saveRDS(seqtab, "yccH_seqtab_from_dada2.rds")
write.csv2(seqtab, "yccH_seqtab_from_dada2.csv")
#14*263-->14*44
seqtab.nochim <- removeBimeraDenovo(seqtab, method="consensus", multithread=TRUE)
saveRDS(seqtab.nochim, "yccH_seqtab_nochim.rds")
write.csv2(seqtab.nochim, "yccH_seqtab_nochim.csv")

#yccH_seqtab_from_dada2.rds
save.image("yccH_seqtab_image.RData")
...
write.csv2(track, "yccH_track.csv")
